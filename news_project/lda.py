...
from helper import word_count, process_text, topic_table, compute_coherence_values, whitespace_tokenizer, unique_words, compute_coherence_values_with_parameters
from split import get_groups
...
path = '/Users/miya/Documents/GitHub/ai4good_news/news_project/test_lda_1/'
test_path = 'test_lda_1/'
...
# best_num_topics = 55
# best_coherence_score = 0.561214788925198
...
for ld in learning_decays:
    ...
    print('learning_decay: {}, coherence: {}'.format(ld,coherence_score))
...
for ld in learning_decays:
    path = '/Users/miya/Documents/GitHub/ai4good_news/news_project/test_lda/{}/'.format(count_params)
    test_path = 'test_lda/{}/'.format(count_params)
    print('learning_decay: {}'.format(ld))
    ...
    lda = LatentDirichletAllocation(
        ...
        learning_decay=ld, # learning rate (default=0.7)
...
params = np.arange(5)
perplexities = list(zip(params,perplexity_scores))
perplexities = sorted(perplexities, key=itemgetter(1), reverse=True)
best_params = perplexities[0][0]
print('test #{} had best params'.format(best_params))
print('perplexity scores: {}'.format(perplexities))
